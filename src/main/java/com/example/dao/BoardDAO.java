...
public class BoardDAO {
	...
	private final String BOARD_INSERT = "insert into BOARD (category, title, writer, content, modified) values (?,?,?,?,NULL)";
...
//	private static final String BOARD_SELECT = "select * from BOARD  where seq=?";
	...
	public BoardVO getPhoto(int seq) {
		BoardVO one = null;
		conn = JDBCUtil.getConnection();
		try {
			stmt = conn.prepareStatement(BOARD_GET);
	...
	public List<BoardVO> getBoardList(){
		...
		try {
			...
			while(rs.next()) {
				...
				one.setModified(rs.getDate("modified"));
				one.setCnt(rs.getInt("cnt"));
				//one.setPhoto(rs.getString("photo"));
				list.add(one);
			}
...
//		try {
//			conn = JDBCUtil.getConnection();
//			stmt = conn.prepareStatement(BOARD_);
//			stmt.setInt(1, seq);
//			rs = stmt.executeQuery();
//			if(rs.next()){
